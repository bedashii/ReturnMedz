...
public class scriptCredits : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        {
            int maxMessageHeightRowMultiplier = 2;
            for (int i = 0; i < Credits.Length; i++)
            {
                messageSize = myStyle.CalcSize(new GUIContent(Credits[i]));
                if (messageSize.x > maxMessageWidth)
                {
                    // If the message is too long, ignore it.
                    if (messageSize.x > Screen.width * 0.9)
                    {
                        maxMessageHeightRowMultiplier++;
                        continue;
                    }
            ...
            maxMessageHeight *= maxMessageHeightRowMultiplier;
        }

        float startPosition = Screen.height * 0.4f;
        float spaceUsed = 0;

        for (int i = 0; i < Credits.Length; i++)
        {
            GUI.Label(new Rect((Screen.width / 2) - (maxMessageWidth / 2), startPosition + spaceUsed - (26 / 2), maxMessageWidth, maxMessageHeight), Credits[i]);
            spaceUsed += 26 + 2;
        }
